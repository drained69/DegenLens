  ...
  return (
    <div className="space-y-6">
      ...
      <Panel title="Attribution claims" subtitle="Registry evidence is separate from observed chain activity" actions={<EvidenceClass kind="inferred" />}>
        {!operator?.wallets?.length ? <p className="text-sm text-slate-500">No address claims are available.</p> : <div className="divide-y divide-ink-700">{operator.wallets.map((wallet) => <div key={`${wallet.address}-${wallet.chain}`} className="grid gap-3 py-4 first:pt-0 last:pb-0 sm:grid-cols-[1fr_auto]"><div><div className="break-all font-mono text-xs text-white">{wallet.address}</div><div className="mt-1 text-xs text-slate-500">{wallet.chain} seed / {wallet.role} / reviewed {wallet.last_reviewed}</div>{wallet.label ? <div className="mt-1 text-xs text-slate-400">Source label: {wallet.label}{typeof wallet.source_confidence === 'number' && wallet.source_confidence > wallet.confidence ? ` (source confidence ${wallet.source_confidence}, capped to ${wallet.confidence} by evidence class)` : ''}</div> : null}<p className="mt-2 text-xs text-slate-500">Queried on {(s.chains_queried ?? operator.queried_chains ?? [wallet.chain]).join(', ')}. {wallet.evidence.length ? `${wallet.evidence.length} evidence source(s)` : 'No supporting source attached. Treat as an investigative seed, not a verified ownership claim.'}</p></div><ConfidenceBadge value={wallet.confidence} status={wallet.evidence_status} /></div>)}</div>}
      </Panel>
